...
func (nh *nodeHeap) getNodeAntColony(meetsConstraints func(*NodeInfo) bool, citys []City) (*api.Node, int) {
	var bestNode *api.Node
	var maxPheromon float64 = 0
	minTasks := int(^uint(0) >> 1) // max int

	for i := 0; i < len(nh.heap); i++ {
		heapEntry := &nh.heap[i]
		if meetsConstraints(heapEntry) && citys[i].pheromon > maxPheromon {
			bestNode = heapEntry.Node
			maxPheromon = citys[i].pheromon
