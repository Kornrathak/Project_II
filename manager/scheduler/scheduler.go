...
func (s *Scheduler) antColony() error {
	...
	for _, a := range citys {
		for _, n := range s.nodeHeap.heap {
			if a.nodeInfo.ID == n.ID {
				s.nodeHeap.Swap(a.index, s.nodeHeap.index[n.ID])
				break
			}
		}
		errP := s.updatePheromon()
...
func (s *Scheduler) updatePheromon() error {
	...
	for i := 0; i < s.nodeHeap.Len(); i++ {
		...
		citys[i].nodeInfo = &s.nodeHeap.heap[i]
		citys[i].index = i
	}
	return nil
}
...
func (s *Scheduler) buildNodeHeap(tx store.ReadTx, tasksByNode map[string]map[string]*api.Task) error {
	...
	for _, n := range nodes {
		...
		node := newNodeInfo(n, tasksByNode[n.ID], resources)
		s.nodeHeap.heap = append(s.nodeHeap.heap, node)
...
func (s *Scheduler) randomStrategy() {
	fmt.Println("Random")
	len := rand.New(rand.NewSource(time.Now().UTC().UnixNano()))
	if s.nodeHeap.Len() > 1 {
		for i := s.nodeHeap.Len(); i > 0; i-- {
			j := len.Intn(i + 1)
			s.nodeHeap.Swap(i, j)
		}
	}
